...
import { useSimCategories, SimCategory } from '@/hooks/useSimCategories'
...
const SWEEP_OPTIONS = ['none', 'top_n', 'weighting', 'rebalance'] as const
    ...
    <div className="bg-black border border-white/10 rounded p-4 mb-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="flex-1 min-w-[200px]">
          <label className="text-[10px] text-white/40 font-mono uppercase block mb-1">Category</label>
          <select
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div>
          ...
          <div className="flex">
            {SWEEP_OPTIONS.map(s => (
              ...
              >
                {s === 'none' ? 'None' : s === 'top_n' ? 'Top N' : s === 'weighting' ? 'Weight' : 'Rebal.'}
              </button>
            ))}
          </div>
        </div>
        <button
          ...
          onClick={onRun}
          disabled={isLoading || !filters.category_id}
        >
