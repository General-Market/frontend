...
interface MarketAccordionProps {
  ...
  /** Reverse lookup: bytes32 hex → resolved market info */
 ...
 */
export function MarketAccordion({ batch, history, bets, onToggleBet, onBulkBet, resolvedMarkets }: MarketAccordionProps) {
  const [expandedCategory, setExpandedCategory] = useState<string | null>(null)

  // Categorize using resolved asset IDs when available, fall back to raw bytes32
  const categories = useMemo(() => {
    if (resolvedMarkets && resolvedMarkets.size > 0) {
      // Map bytes32 → resolved assetId for categorization
      const resolvedIds = batch.marketIds.map(id => {
        const resolved = resolvedMarkets.get(id.toLowerCase())
        return resolved ? resolved.assetId : id
      })
      const cats = categorizeMarkets(resolvedIds)
      // Re-map categories back to bytes32 IDs (needed for bets keying)
      const assetToHash = new Map<string, string>()
      batch.marketIds.forEach(id => {
        const resolved = resolvedMarkets.get(id.toLowerCase())
        if (resolved) assetToHash.set(resolved.assetId, id)
      })
      return cats.map(cat => ({
        ...cat,
        markets: cat.markets.map(m => assetToHash.get(m) || m),
      }))
    }
    return categorizeMarkets(batch.marketIds)
  }, [batch.marketIds, resolvedMarkets])

  // Latest prices from snapshot (preferred) or from tick history (fallback)
  const latestPrices = useMemo(() => {
    const result: Record<string, { price: number; change: number }> = {}

    // First, fill from resolved markets (live snapshot data)
    if (resolvedMarkets) {
      for (const [hash, market] of resolvedMarkets) {
        result[hash] = {
          price: market.value,
          change: market.changePct ?? 0,
        }
      }
    }

    // Override with tick history if available (more relevant for the batch context)
    if (history.length > 0) {
      ...
      <table className="w-full text-xs font-mono">
        ...
        <tbody>
          {markets.map(marketId => {
            const priceInfo = latestPrices[marketId] ?? latestPrices[marketId.toLowerCase()]
            const mktHistory = (marketHistory[marketId] || []).slice(-6)
            const bet = bets[marketId]
            const isUp = bet === true
            const resolved = resolvedMarkets?.get(marketId.toLowerCase())

            // Display name: resolved name or formatted market ID
            const displayName = resolved?.name || formatMarketName(marketId)
                ...
                <td className="py-1 px-2 max-w-[260px]">
                  {sourceUrl ? (
                    <a
                      ...
                      className="text-text-primary hover:text-terminal transition-colors truncate block"
                      title={displayName}
                    >
                      {displayName}
                    </a>
                  ) : (
                    <span className="text-text-primary truncate block" title={displayName}>
                      {displayName}
                    </span>
                  )}
                </td>
...
/** Format price for display — use compact notation for large numbers, hide $ for scores */
function formatPrice(price: number): string {
  ...
  // Small values (scores, counts, etc.) - show raw number
  if (price < 1 && price > 0) {
