...
export function PythonEditor({ batch, onBitmapGenerated }: PythonEditorProps) {
  ...
  function handleBacktest() {
    if (!previewBitmap) {
      backtest.reset()
      // Trigger an error state so the user sees feedback
      backtest.mutate({ batchId: batch.id, bitmapHex: '', ticks: 50 })
      return
    }
    const bitmapHex = encodeBitmapHex(previewBitmap, batch.marketIds)
    backtest.mutate({ batchId: batch.id, bitmapHex, ticks: 50 })
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 items-center">
        ...
        <button
          ...
          disabled={backtest.isPending || !previewBitmap}
          title={!previewBitmap ? 'Run preview first to generate bitmap' : undefined}
...
/** Encode a bitmap record into packed hex format for the backtest API */
function encodeBitmapHex(bitmap: Record<string, boolean>, marketIds: string[]): string {
  const byteCount = Math.ceil(marketIds.length / 8)
  const bytes = new Uint8Array(byteCount)
  for (let i = 0; i < marketIds.length; i++) {
    if (bitmap[marketIds[i]] === true) {
