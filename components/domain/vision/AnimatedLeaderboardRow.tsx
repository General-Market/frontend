...
import { useRankChangeAnimation } from '@/hooks/vision/useRankChangeAnimation'
import { AnimatedNumber } from '@/components/ui/AnimatedNumber'
import { AgentRanking } from '@/hooks/vision/useLeaderboard'
...
/**
 * AnimatedLeaderboardRow component (vision/BlackRock light theme)
 *
...
export function AnimatedLeaderboardRow({
  ...
  const pnlColor = agent.pnl >= 0 ? 'text-color-up' : 'text-color-down'
  const roiColor = agent.roi >= 0 ? 'text-color-up' : 'text-color-down'

  // Track rank change animations for this agent
  const { isAnimating } = useRankChangeAnimation(agent.walletAddress)
  ...
  // Animation class for rank change (2s duration)
  const rankChangeClass = isAnimating ? 'bg-color-up/5' : ''
  ...
  return (
    <motion.tr
      ...
      className={`cursor-pointer hover:bg-surface focus:outline-none focus:ring-2 focus:ring-brand/30 transition-colors duration-300 ${highlightClass} ${rankChangeClass}`}
    ...
    >
      ...
      <TableCell className={`font-mono hidden md:table-cell ${(agent.unrealizedPnl ?? 0) >= 0 ? 'text-text-secondary' : 'text-color-warning'}`}>
        <AnimatedNumber
      ...
      <TableCell className="font-mono text-text-secondary hidden md:table-cell">
        {(agent.totalBets ?? 0).toLocaleString()}
      </TableCell>

      {/* Avg Portfolio Size - animated */}
      <TableCell className="font-mono text-text-secondary font-bold hidden md:table-cell">
      ...
      {/* Max Portfolio - animated */}
      <TableCell className="font-mono text-brand font-bold">
